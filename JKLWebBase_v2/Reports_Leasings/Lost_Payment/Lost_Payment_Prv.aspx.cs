...
{
    public partial class Lost_Payment_Prv : Page
    {
        ...
        protected void Export_Reported_Btn_Click(object sender, EventArgs e)
        {
            ...
            string name = string.IsNullOrEmpty(Cust_Name_TBx.Text) ? "" : Cust_Name_TBx.Text.Trim(' ');
            ...
            Session["fname_rpt"] = string.IsNullOrEmpty(name) ? "" : name.Split(' ')[0];
            Session["lname_rpt"] = name.IndexOf(' ') <= 0 ? "" : name.Split(' ')[1];
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "script", "<script type = 'text/javascript'> window.open('/Reports_Leasings/Lost_Payment/Lost_Payment_Export'); </script>");
        }
