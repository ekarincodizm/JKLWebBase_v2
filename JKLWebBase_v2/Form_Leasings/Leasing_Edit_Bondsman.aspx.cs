...
{
    public partial class Leasing_Edit_Bondsman : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if(Session["Bondsman_" + nob] != null)
                {
                    Customers load_ctm  = (Customers)Session["Bondsman_" + nob];
        ...
        {
            nob = Convert.ToInt32(Session["Number_Of_Bondsman"].ToString());
            ctm = (Customers)Session["Bondsman_" + nob];
            cls = (Car_Leasings)Session["Leasings"];

            _RemoveBondsmans(cls.Leasing_id, nob, ctm.Cust_id);

            _AddCustomer();

            Session.Remove("chk_bsm");
            Session.Remove("chk_bsm_spouse");
...
*******************************************************************************************************************************************************************************/
        ...
        *******************************************************************************************************************************************************************************/
        ...
        private bool _RemoveBondsmans(string Leasing_id, int bondsman_no, string Cust_id)
        {
            return new Car_Leasings_Manager().removeBondsman(Leasing_id, bondsman_no, Cust_id);
        }
    }
}