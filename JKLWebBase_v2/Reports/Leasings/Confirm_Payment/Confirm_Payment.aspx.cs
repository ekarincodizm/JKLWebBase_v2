...
namespace JKLWebBase_v2.Reports.Leasings.Confirm_Payment
{
    public partial class Confirm_Payment : Page
    {
        ...
        private Leasing_Ds _loadReport()
        {
            ...
            {
                error = "MysqlException ==> Confirm_Payment --> _loadReport() : " + ex.Message.ToString();
                Log_Error._writeErrorFile(error);
                return null;
            }
            catch (Exception ex)
            {
                error = "Exception ==> Confirm_Payment --> _loadReport() : " + ex.Message.ToString();
                Log_Error._writeErrorFile(error);
                return null;
            }
            finally
        ...
        public void printReportFull(Leasing_Ds ls_ds)
        {
            ...
            Confirm_Payment_Full rpt = new Confirm_Payment_Full();
            rpt.SetDataSource(ls_ds);
        ...
        public void ExportReportFull(Confirm_Payment_Full rpt)
        {
            cls = (Car_Leasings)Session["Leasings"];
            ...
            /// Export Report to PDF File with Save As Mode
            rpt.ExportToDisk(ExportFormatType.PortableDocFormat, @"C:/ReportExport/" + mainDirectory + "/Confirm_Payment_Full_" + cls.Deps_no + ".pdf");

            /// Display PDF File to PDF Program
            Process process = new Process();
            process.StartInfo.UseShellExecute = true;
            process.StartInfo.FileName = "C:/ReportExport/" + mainDirectory + "/Confirm_Payment_Full_" + cls.Deps_no + ".pdf";
            process.Start();
        }
    }
}